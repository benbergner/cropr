...
def merge_windows(windows, window_size, ori_shape, merge_type):
    ws = window_size
    im_windows = windows[merge_type]
    anchors = windows["anchors"]
    C = im_windows[0].shape[0]
    H, W = windows["seg_maps"].shape[2:]
    flip = windows["flip"]
    ...
    result = F.softmax(logit, 0) if merge_type == "seg_maps" else logit
    return result


def inference(model, ims, ims_metas, ori_shape, window_size, window_stride, batch_size):
    """
    We resize the max edge to 512 px and pad the smaller edge
    while maintaining the aspect ratio. This 1-shot evaluation
    approach is optimized for throughout and is more challenging
    than the common single-scale evaluation setting, which
    averages predictions from a sliding window.
    """
    ...
    for im, im_metas in zip(ims, ims_metas):
        ...
        windows["seg_maps"] = windows["seg_maps"][
            :, :, : ims_metas[0]["img_shape"][0], : ims_metas[0]["img_shape"][1]
        ]

        im_seg_map = merge_windows(windows, window_size, ori_shape, "seg_maps")
        seg_map += im_seg_map
